...
var Orb = {
	...
	stepSize: 360,
	drawGrid: 1, 			// Boolean: Show/Hide grid
	view_sun: 1, 			// Boolean: Show/Hide sun
	view_tracks: 1, 		// Boolean: Show/Hide satellite paths
	view_multi: 1,
	view_foot: 1, 
	sat_square: 6, 			// Edge length of satellite position indicator squares
	sat_color: "#FFFFFF", 	// Satellite display color
	gs_color: "#FF0000", 	// Ground station display color
	sat_eclipse: "#DDDDDD", // Satellite eclipse zone color 
	...
	*/
	...
	/*
	Set default satellite position.
	*/
	pos_t: function(){
		this.lat = 0;
		this.lon = 0;
	...
	/*
	Load the TLE data from the specified URL.
	
	@param url: URL of the TLE source.
	*/
	...
	/*
	Generate the canvas map.
	
	@param canvas: Canvas object to load into.
	@param url: URL of the background image (map file).
	*/
	drawMap: function(canvas, url){
		...
		// Insert to image into the canvas object once it loads
		img.onload = function(){
	...
	*/
	loadTLEs: function(){
		...
		if (Orb.tles != "" ){
			// Split up the TLE input file
			var lines = Orb.tles.split('\n');
			entries = Math.floor(lines.length / 3); // 3 lines per TLE entry
			for (var current_line = 0; current_line < entries; current_line++){
				// Load the TLE entry into PLib
				name  = lines[current_line*3+0].replace(/^\s+|\s+$/g, '');
				line1 = lines[current_line*3+1];
				line2 = lines[current_line*3+2];
				PLib.tleData[current_line] = new Array(3);
				PLib.tleData[current_line][0] = name;
				PLib.tleData[current_line][1] = line1;
				PLib.tleData[current_line][2] = line2;
			}
	...
	/*
	Set's the location of the home ground station.
	
	@param homeLat: Home ground station latitude.
	@param homeLng: Home ground station longitude.
	*/
	setHomeCoordinates: function(homeLat, homeLng){
		// Set the home ground station location in PLib
		PLib.configureGroundStation(homeLat, homeLng);
		
		// Set the location of the home ground station indicator
		Orb.home.style.top = ((-homeLat + 90) * 1.5 - 12.0) + "px";
		Orb.home.style.left =  ((parseInt(homeLng) + 180) * 1.5 - 12.0) + "px";
	},
	
	/*
	Cross browser method of setting CSS styles.
	*/
	...
	/*
	Creates individual satellite markers.
	
	@param txt: Index of the satellite currently being processed.
	*/
	...
	/*
	Creates indicators for all displayed satellites.
	*/
	...
	/*
	Main satellite update loop. Positions each satellite indicator every 5 seconds based on the position calculated by PLib.
	*/
	...
	*/
	...
	/*
	Creates a new table column header.
	
	@param tr: The row to append the column too.
	@param txt: Text to use as the column header.
	*/
	...
	/*
	Generates primary display table.
	
	@param divTable: The container to insert the table into.
	*/
	...
	/*
	Updates the clock when needed.
	
	@param delay: How many seconds to wait before updating the clock.
	*/
	...
	/*
	Generates the satellite position bar for the specified satellite.
	
	@param sat_name: Name of the satellite to render position bar for.
	*/
	...
	/*
	Generates the main satellite position map.
	
	@param divCanvas: The HTML5 canvas to load the map into.
	*/
	...
	@param color: Desired grid color.
	*/
	...
	@param index: Setting to toggle (e.g. "sun", "tracks")
	@param cell: 
	*/
	...
	/*
	Used to plot ground stations.
	
	@param ctx: HTML5 canvas context.
	@param lat: Ground station latitude.
	@param lon: Ground station longitude.
	@param sat_name: Ground station name.
	*/
	...
	/*
	Plots satellie positions.
	
	@param sat: Satellite object to plot.
	@param ctx: HTML5 canvas context.
	@param name: Unused.
	*/
